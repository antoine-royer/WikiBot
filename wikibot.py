...
class WikiBot(commands.Cog):
    ...
    @slash.slash(name="r", description="Random selection of articles from Wikipedia", guild_ids=self.guild_ids)
    ...
    @slash.slash(name="p-auto", description="Get an article with an automatic correction on the title", guild_ids=self.guild_ids)
    async def pauto(ctx, research: str, language: str="en"):
        wl.wikipedia.set_lang(language.split()[0])
        await ctx.send(embed=make_embed(*wl.page_read(research, True)))


    @slash.slash(name="p", description="Get an article from Wikipedia with the exact title", guild_ids=self.guild_ids)
    async def p(ctx, research: str, language: str="en"):
        wl.wikipedia.set_lang(language.split()[0])
        await ctx.send(embed=make_embed(*wl.page_read(research)))


    @slash.slash(name="s", description="Make a research on wikipedia", guild_ids=self.guild_ids)
    async def s(ctx, research: str, language: str="en"):
        wl.wikipedia.set_lang(language.split()[0])
        await ctx.send(embed=make_embed(*wl.page_search(research)))


    @slash.slash(name="w", description="Get the weather", guild_ids=self.guild_ids)
    ...
    @slash.slash(name="n", description="Get some news", guild_ids=self.guild_ids)
